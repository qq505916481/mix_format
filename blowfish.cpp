...
static inline dword reverse(dword v)
{
    // mov 将v赋值给eax寄存器
    // xchg交换al与ah寄存器的值
    // rol位操作左移，将eax的二进制值左移16位
    // 寄存器的关系：
    // eax是32位的寄存器，ax是eax的低16位（倒数最后16位）
    // ah则是ax的高8位，al是ax的低8位
    asm ("movl %1, %%eax;"
         "xchg %%ah, %%al;"
         "rol $16, %%eax;"
         "xchg %%ah, %%al;"
...
/*     dword temp = (v & 0xffff0000) | ((v & 0x0000ff00) >> 8) | ((v & 0x000000ff) << 8);
    // rol : (temp << num) | temp >> (sizeof(temp)-num))
    temp = (temp << 16) | (temp >> 16);
    temp = (temp & 0xffff0000) | ((temp & 0x0000ff00) >> 8) | ((temp & 0x000000ff) << 8);
    v = temp; */
