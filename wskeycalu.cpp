...
int main(int argc, char** argv)
{
    if(argc < 2)
	  {
		  printf("usage: wskeycalu <file>\n");
		  printf(" file: mix files to check.\n");
		  return 1;
	  }
    ...
    inFile.read((char*) input_key, 80);

    cout << "Input key: " << endl;
    for(int i=0; i< 80; i++)
        cout << hex << setfill('0') << setw(2) << (int) input_key[i] << " ";
    cout << endl;
    
    get_blowfish_key(input_key, output_key);

    cout << "Output key: " << endl;
    for(int i=0; i< 56; i++)
        cout << hex << setfill('0') << setw(2) << (int) output_key[i] << " ";
    cout << endl;


    // read encrypted 8 bytes
    inFile.seekg(84, ios::beg);
    byte encrypted_data[8];
    byte decrypted_data[8];
    
    inFile.read((char*) encrypted_data, 8);
    cout << "Encrypted MIX header: " << endl;
    for(int i=0; i< 8; i++)
        cout << hex << setfill('0') << setw(2) << (int) encrypted_data[i] << " ";
    cout << endl;

    Cblowfish blowfish;
    blowfish.set_key(output_key, 56);
    blowfish.decipher(encrypted_data, decrypted_data, 8);

    cout << "Decrypted MIX header: " << endl;
    for(int i=0; i< 8; i++)
        cout << hex << setfill('0') << setw(2) << (int) decrypted_data[i] << " ";
    cout << endl;

    // Recrypt Test 07 01 34 a4 15 0d‬ 89 dc
    byte recrypted_data[8] = {0x07, 0x01 ,0x34, 0xa4, 0x15, 0x0d, 0x89, 0xdc};

    cout << "New MIX header (before): " << endl;
    for(int i=0; i< 8; i++)
        cout << hex << setfill('0') << setw(2) << (int) recrypted_data[i] << " ";
    cout << endl;

    blowfish.encipher(recrypted_data, recrypted_data, 8);
    cout << "Recrypted MIX header: " << endl;
    for(int i=0; i< 8; i++)
        cout << hex << setfill('0') << setw(2) << (int) recrypted_data[i] << " ";
    cout << endl;
